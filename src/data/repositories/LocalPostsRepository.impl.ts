...
export class LocalPostsRepository implements PostsRepository {
  ...
  async like(id: string): Promise<void> {
    const list = await this.list();
    LocalStore.setJSON(KEY, list.map(p => p.id === id ? { ...p, isLiked: true, likesCount: p.likesCount + 1 } : p));
  }
  async unlike(id: string): Promise<void> {
    const list = await this.list();
    LocalStore.setJSON(KEY, list.map(p => p.id === id ? { ...p, isLiked: false, likesCount: Math.max(0, p.likesCount - 1) } : p));
  }
}
